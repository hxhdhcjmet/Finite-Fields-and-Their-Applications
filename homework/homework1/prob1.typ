...
(d)由$f | m$设$m = f d$,对$a^n in <a>,0<= n < m$满足$a^n$的阶为$f$,有:
$  |<a^n>| = |a^n| = m/gcd(m,n) = f $
故$gcd(m,n) = m/f = d$,因此有$d | n$,不妨令$n = r d,0<=r<f$,则:
$  gcd(m,n) = gcd(f d,r d) = d dot gcd(f,r) = d $
因此$ gcd(f,r) = 1$,这样的$r$有$phi.alt(f)$个，所对应的$a^(n)(r d = n)$满足$0<=n<m$，因此$a^n$各不相同，共有$phi.alt(f)$个。因此$<a>$包含$phi.alt(f)$个阶为$f$的元素，得证。

(e)对于$a^n in <a>$为$<a>$的生成元，则$<a>$的阶为$m$，由(b)知:
$   |a^n| = |<a^n>| = m /gcd(m,n) = m  $
因此$gcd(m,n) = 1$，这样的$n$恰有$phi.alt(m)$个，且这些生成元为$a^r$，其中满足$gcd(r,m) = 1$，得证。
]